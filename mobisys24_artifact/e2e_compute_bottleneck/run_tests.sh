...
batch_list=(1 8)
policy_list=("conventional" "spinn" "fl" "dynamic")
server_core_list=(64 32 16 8 4)
bandwidth=100
num_tile=200
...
echo "    Server core settings: ${server_core_list[@]}"
echo "    Server core settings: ${server_core_list[@]}" >> $output_log
echo ""
echo "Script started at $start_time"
echo "Script started at $start_time" >> $output_log
echo ""
echo "" >> $output_log
echo "EDGE_ip,Scheduling Policy,DNN,Batch size,SERVER Core,BW,Total latency (ms),Transmission latency (ms)" >> $output_csv

total_runs=$(( ${#dnn_list[@]} * ${#batch_list[@]} * ${#policy_list[@]} * ${#server_core_list[@]} ))
...
do
    ...
    do
        for s_core in "${server_core_list[@]}"
        do
            ...
            do
                ...
                if [ "$policy" != "fl" ]; then
                    ...
                    server_cmd="taskset -c 0-$(($s_core+1)) ../../coacto --device_mode=0 --dirname=$dir_name --target_nasm_dir="../../data/$nasm_file" --target_dnn_dir="../../data/${dnn}_base.aspen" --target_input="../../data/batched_input_128.bin" --prefix="$dnn" --server_ip="$server_ip"  --server_port="$server_port"  --schedule_policy="$policy" --sched_sequential_idx=1 --dse_num=64 --output_order="${output_format}" --inference_repeat_num=$num_runs --num_edge_devices=1"
                    edge_cmd_wo_ssh="../../coacto --device_mode=1 --dirname=$dir_name --target_nasm_dir="../../data/$nasm_file" --target_dnn_dir="../../data/${dnn}_base.aspen" --target_input="../../data/batched_input_128.bin" --prefix="$dnn" --server_ip="$server_ip" --server_port="$server_port" --schedule_policy="$policy" --sched_sequential_idx=1 --dse_num=6 --output_order="${output_format}" --inference_repeat_num=$num_runs --num_edge_devices=1"
                else
                    ...
                    if [ "$dnn" == "bert_base" ]; then
                        server_cmd="taskset -c 0-$(($s_core+1)) ../../coacto --device_mode=0 --dirname=$dir_name --target_nasm_dir="../../data/$nasm_file" --target_dnn_dir="../../data/${dnn}_base.aspen" --target_input="../../data/batched_input_128.bin" --prefix="$dnn" --server_ip="$server_ip"  --server_port="$server_port"  --schedule_policy=conventional --sched_sequential_idx=1 --dse_num=64 --output_order="${output_format}" --inference_repeat_num=$num_runs --num_edge_devices=1"
                        edge_cmd_wo_ssh="../../coacto --device_mode=1 --dirname=$dir_name --target_nasm_dir="../../data/$nasm_file" --target_dnn_dir="../../data/${dnn}_base.aspen" --target_input="../../data/batched_input_128.bin" --prefix="$dnn" --server_ip="$server_ip" --server_port="$server_port" --schedule_policy=conventional --sched_sequential_idx=1 --dse_num=6 --output_order="${output_format}" --inference_repeat_num=$num_runs --num_edge_devices=1"
                    fi
                fi

                edge_cmd="$shell_cmd 'cd ~/kmbin/CoActo/mobisys24_artifact/e2e_compute_bottleneck && $edge_cmd_wo_ssh'"
                ...
                echo "$edge_ip,$policy,$dnn,$batch,$s_core,$bandwidth,$avg_edge_time,$avg_transmission_latency" >> $output_csv
