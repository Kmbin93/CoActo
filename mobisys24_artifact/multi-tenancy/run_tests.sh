if [ "$#" -ne 3 ]; then
    ...
    echo "Usage: ./run_tests.sh <num_runs> <server_ip> <server_port>"
    exit 1
fi
...
edge_uname_list=("nxc" "nxc")
edge_passwd_list=("bestnxcl" "NXCfromMSN2020!")
edge_ip_list=("147.46.118.129" "147.46.130.75")
edge_port_list=(56788 61103)

dnn_list=("vgg16" "resnet50" "bert_base" "yolov3")
batch=1
policy_list=("conventional" "spinn" "dynamic")
s_core=64
bandwidth=100
num_tile=200
num_edge_devices=${#edge_ip_list[@]}
...
total_runs=$(( ${#dnn_list[@]} * ${#policy_list[@]}))

# Set TC in EDGE
for i in $(seq 0 $((num_edge_devices-1)))
do
    edge_passwd=${edge_passwd_list[i]}
    shell_cmd="ssh ${edge_uname_list[i]}@${edge_ip_list[i]} -p ${edge_port_list[i]}"
...
do    
    ...
    do
        ...
        if [ "$policy" != "fl" ]; then
            ...
            server_cmd="taskset -c 0-$(($s_core+1)) ../../coacto --device_mode=0 --dirname=$dir_name --target_nasm_dir="../../data/$nasm_file" --target_dnn_dir="../../data/${dnn}_base.aspen" --target_input="../../data/batched_input_128.bin" --prefix="$dnn" --server_ip="$server_ip"  --server_port="$server_port"  --schedule_policy="$policy" --sched_sequential_idx=1 --dse_num=64 --output_order="${output_format}" --inference_repeat_num=$num_runs --num_edge_devices=$num_edge_devices"
            edge_cmd_wo_ssh="../../coacto --device_mode=1 --dirname=$dir_name --target_nasm_dir="../../data/$nasm_file" --target_dnn_dir="../../data/${dnn}_base.aspen" --target_input="../../data/batched_input_128.bin" --prefix="$dnn" --server_ip="$server_ip" --server_port="$server_port" --schedule_policy="$policy" --sched_sequential_idx=1 --dse_num=6 --output_order="${output_format}" --inference_repeat_num=$num_runs --num_edge_devices=$num_edge_devices"
        else
            ...
            if [ "$dnn" == "bert_base" ]; then
                server_cmd="taskset -c 0-$(($s_core+1)) ../../coacto --device_mode=0 --dirname=$dir_name --target_nasm_dir="../../data/$nasm_file" --target_dnn_dir="../../data/${dnn}_base.aspen" --target_input="../../data/batched_input_128.bin" --prefix="$dnn" --server_ip="$server_ip"  --server_port="$server_port"  --schedule_policy=conventional --sched_sequential_idx=1 --dse_num=64 --output_order="${output_format}" --inference_repeat_num=$num_runs --num_edge_devices=$num_edge_devices"
                edge_cmd_wo_ssh="../../coacto --device_mode=1 --dirname=$dir_name --target_nasm_dir="../../data/$nasm_file" --target_dnn_dir="../../data/${dnn}_base.aspen" --target_input="../../data/batched_input_128.bin" --prefix="$dnn" --server_ip="$server_ip" --server_port="$server_port" --schedule_policy=conventional --sched_sequential_idx=1 --dse_num=6 --output_order="${output_format}" --inference_repeat_num=$num_runs --num_edge_devices=$num_edge_devices"
            fi
        fi
        ...
        do
            shell_cmd="ssh ${edge_uname_list[i]}@${edge_ip_list[i]} -p ${edge_port_list[i]}"
            edge_cmd="$shell_cmd 'cd ~/kmbin/CoActo/mobisys24_artifact/multi-tenancy && $edge_cmd_wo_ssh'"
        ...
        do
            ...
            echo "${edge_ip_list[i]},$policy,$dnn,$batch,$s_core,$bandwidth,$num_tile,$avg_edge_time,$avg_transmission_latency" >> $output_csv
        done
        sleep 1            
    done
done